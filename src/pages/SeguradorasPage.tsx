...
export default function SeguradorasPage() {
  ...
  const filtered = seguradoras?.filter((seg) => {
    const matchesSearch = seg.razao_social.toLowerCase().includes(search.toLowerCase()) ||
      (seg.cnpj && seg.cnpj.includes(search)) ||
      (seg.nome_fantasia && seg.nome_fantasia.toLowerCase().includes(search.toLowerCase()));
    const matchesStatus = statusFilter === 'todos' || seg.status === statusFilter;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie as seguradoras parceiras</p>
        </div>
        <Button asChild><Link to="/cadastros/seguradoras/novo"><Plus className="mr-2 h-4 w-4" />Nova Seguradora</Link></Button>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">Nenhuma seguradora encontrada</TableCell></TableRow>
            ) : (
              paginated.map((seg) => (
                <TableRow key={seg.id}>
                  <TableCell className="font-medium">{seg.razao_social}</TableCell>
                  <TableCell>{seg.nome_fantasia || '-'}</TableCell>
                  <TableCell>{seg.cnpj || '-'}</TableCell>
                  <TableCell>{seg.telefone || '-'}</TableCell>
                  <TableCell>{seg.email || '-'}</TableCell>
                  <TableCell>
                    <Badge variant={seg.status === 'ativo' ? 'default' : 'secondary'} className={seg.status === 'ativo' ? 'bg-green-100 text-green-700' : ''}>
